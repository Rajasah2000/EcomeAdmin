...
const AndAndManageNewsCategory = () => {
  ...
  const AddCategory = async () => {
    ...
    if (name && description && image) {
      const res = await HomeService.AddNewsCategory(data);
      if (res && res.status) {
  ...
  const fetchViewCategory = async () => {
    setLoader(true);
    const res = await HomeService.ViewAllNewsCategory();
  ...
  const onDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        HomeService.DeleteNewsCategory(id)
          .then((res) => {
            if (res && res.status) {
  ...
  const UpdateLearningCategory = () => {
    ...
    if (name && image && description) {
      HomeService.UpdateNewsCategory(id, data)
        .then((res) => {
          if (res && res.status) {
          ...
          <div className="main-card mb-3 card">
            <div className="card-body">
              ...
              {hide ? (
                ...
                <button
                  class="btn btn-primary"
                  onClick={UpdateLearningCategory}
                  style={{ marginTop: "1rem" }}
                >
                  Update
                </button>
...
export default AndAndManageNewsCategory