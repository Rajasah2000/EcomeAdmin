...
const HandleAddProduct = async(e) => {
    ...
    if(categoryId && productName && description  &&price && quantity &&image &&offerpercent &&rating){
        try {
       const res = await Helpers('http://localhost:3004/api/admin/add-product', 'POST' , data);
       if(res && res?.status){
          ...
          <form enctype="multipart/form-data">
            <div class="form-row" style={{ marginBottom: "9px" }}>
              <div class="col">
                ...
                <select value={categoryId} onChange={(e) => setCategoryId(e.target.value)} id="inputState" class="form-control">
                  <option selected>Choose category ...</option>
                  {categoryData?.map((ele , index) => {
                    return(
                        <option value={ele?._id} key={index}>{ele?.name}</option>
                    )
                  })}
                </select>
              </div>
