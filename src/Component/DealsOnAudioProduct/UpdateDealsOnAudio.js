...
const UpdateDealsOnAudio = () => {
  ...
  useEffect(() => {
    if (location?.pathname === "/edit-deals-product") {
  ...
  const HandleUpdateProduct = async (e) => {
    ...
    //   categoryId &&
      ...
      try {
        const res = await Helpers(
          `http://localhost:3004/api/admin/update-deals-product/${location?.state?._id}`,
          "PUT",
          data
        );
        if (res && res?.status) {
...
export default UpdateDealsOnAudio
