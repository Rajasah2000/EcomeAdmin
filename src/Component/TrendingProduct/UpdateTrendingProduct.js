...
const UpdateTrendingProduct = () => {
  ...
  useEffect(() => {
    if (location?.pathname === "/edit-trending-product") {
  ...
  const HandleUpdateProduct = async (e) => {
    ...
    //   categoryId &&
      ...
      try {
        const res = await Helpers(
          `http://localhost:3004/api/admin/update-tranding-product/${location?.state?._id}`,
          "PUT",
          data
        );
        if (res && res?.status) {
          ...
          file.value = "";
          clearAllState();
          navigate("/manage-trending-product");
        } else {
          toast.error(res?.msg);
        }
      } catch (error) {
...
export default UpdateTrendingProduct
