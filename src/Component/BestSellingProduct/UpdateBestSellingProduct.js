...
const UpdateBestSellingProduct = () => {
  ...
  useEffect(() => {
    if (location?.pathname === "/edit-best-product") {
  ...
  const HandleUpdateProduct = async (e) => {
    ...
    //   categoryId &&
      ...
      try {
        const res = await Helpers(
          `http://localhost:3004/api/admin/update-selling-product/${location?.state?._id}`,
          "PUT",
          data
        );
        if (res && res?.status) {
...
export default UpdateBestSellingProduct
